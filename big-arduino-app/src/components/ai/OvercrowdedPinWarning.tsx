 ...
 * whenever two or more wires/component-pins share the same physical hole.
...
function findFirstOvercrowdedKey(
  ...
  // Component legs inserted into a breadboard
  ...
  // Build a fast lookup: componentId → its inserted-pin map
  const insertedPinsLookup = new Map<string, { parentBreadboardId: string; pins: Record<string, string> }>();
  for (const comp of placedComponents) {
    if (comp.parentBreadboardId && comp.insertedPins) {
      insertedPinsLookup.set(comp.instanceId, {
        parentBreadboardId: comp.parentBreadboardId,
        pins: comp.insertedPins,
      });
    }
  }
  ...
  for (const wire of wires) {
    ...
    // Case B: wire endpoint is a component pin that's inserted in a breadboard.
    // The wire physically shares that breadboard hole with the component leg.
    const startEntry = insertedPinsLookup.get(wire.startComponentId);
    if (startEntry) {
      const bbPinId = startEntry.pins[wire.startPinId];
      if (bbPinId) inc(startEntry.parentBreadboardId, bbPinId);
    }
    const endEntry = insertedPinsLookup.get(wire.endComponentId);
    if (endEntry) {
      const bbPinId = endEntry.pins[wire.endPinId];
      if (bbPinId) inc(endEntry.parentBreadboardId, bbPinId);
    }
  }
