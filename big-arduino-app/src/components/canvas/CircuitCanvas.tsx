...
/** Sharp 90° polyline — looks like an open “half rectangle” / U (vertical → horizontal → vertical). */
function createOrthogonalPolylinePath(points: { x: number; y: number }[]): string {
  if (points.length < 2) return '';
  let path = `M ${points[0].x} ${points[0].y}`;
...
/**
 * Bend points for rail demo wires: two right-angle corners forming a U (half-rectangle outline).
 * Segment 1: pin → straight down; 2: along bottom; 3: straight up to the other pin.
 * @param runYOffset Shift the horizontal run (px, + = downward) so e.g. 5V vs GND don’t overlap.
 */
function buildLedPowerRailJogBends(
  breadboardPin: { x: number; y: number },
  arduinoPin: { x: number; y: number },
  runYOffset = 0
): { x: number; y: number }[] {
  const jogDown = 52;
  const runY = Math.max(breadboardPin.y, arduinoPin.y) + jogDown + runYOffset;
  return [
    { x: breadboardPin.x, y: runY },
    { x: arduinoPin.x, y: runY },
  ];
}

/** 5V (red) vs GND (black) demo jumpers: separate the shared horizontal segment vertically */
const LED_RAIL_DEMO_RUN_OFFSET_5V = -9;
const LED_RAIL_DEMO_RUN_OFFSET_GND = 9;

/** Dashed wires with bends use orthogonal polylines (half-rectangle); solid wires keep rounded corners. */
function wirePathStringFromPoints(
  wire: { strokeDashArray?: number[] },
  ...
  const dashed = !!(wire.strokeDashArray && wire.strokeDashArray.length > 0);
  return dashed ? createOrthogonalPolylinePath(points) : createRoundedPath(points, 12);
}

/** Instructional wires drawn on top of components; outline under the stroke. */
function moveWirePairAboveComponents(
  canvas: fabric.Canvas,
  outline: fabric.Path | null,
  wire: fabric.Path
) {
  if (outline) {
    canvas.bringObjectToFront(outline);
  }
  canvas.bringObjectToFront(wire);
}

/**
 * When choosing whether a click selects a wire, defer to Fabric’s target if the user hit a
 * placed part (LED, resistor, pushbutton, …). Breadboard / Uno stay “under” the wire for pin UX.
 */
function shouldDeferWireSelectionToComponent(
  canvas: fabric.Canvas,
  mouseEvent: MouseEvent,
  getDef: (instanceId: string) => ComponentDefinition | undefined
): boolean {
  const info = canvas.findTarget(mouseEvent);
  const t = info?.target as ComponentFabricObject | undefined;
  if (!t?.data || t.data.type !== 'component') return false;
  const instanceId = t.data.instanceId;
  if (!instanceId) return false;
  const d = getDef(instanceId);
  if (!d) return false;
  if (d.id === 'breadboard' || d.id === 'arduino-uno') return false;
  return true;
}

/**
 * Half-size breadboard (breadboard.json): bottom − rail centers y≈382, + rail y≈403.
 * Rectangles in component local space (origin top-left), aligned to the colored rail bands.
 */
const BREADBOARD_BOTTOM_RAIL_LOCAL = {
  /** Blue (−) strip */
  minus: { left: 14, top: 366, width: 630, height: 32 },
  /** Red (+) strip */
  plus: { left: 14, top: 387, width: 630, height: 32 },
} as const;

/** Closed SVG path in canvas space covering a local rect (handles rotation/scale like pin positions). */
function breadboardRailLocalRectToCanvasPathD(
  ...
  const pts = corners.map(([lx, ly]) => {
    const p = new fabric.Point(lx - cx, ly - cy);
...
function fallbackOrthogonal(a: { x: number; y: number }, b: { x: number; y: number }): { x: number; y: number }[] {
  ...
  // Vertical first (same convention as orthogonalRouter.orthogonalConnect)
...
/** Snap cursor position for wire *preview* routing — stabilizes orthogonal path while dragging. */
const WIRE_PREVIEW_END_SNAP_PX = 6;
...
interface CircuitCanvasProps {
  ...
  /**
   * Button-Powered LED: after the user clicks “Next” on the banner (Frame 3),
   * auto-place dashed 30% power jumpers and bottom-rail overlays:
   * breadboard bottom (+) hole 14 → Uno 5V, bottom (−) hole 16 → Uno GND1.
   */
  ledButtonFrame3PowerDemo?: boolean;
  /** Button-Powered LED Frame 8: dim canvas objects to 40% except the target LED. */
  ledButtonFrame8DimWorkspace?: boolean;
  /** Instance id of the real user LED (G–J zone); must match when dimming is on. */
  ledButtonFrame8TargetInstanceId?: string | null;
  /** Frame 11: dim workspace to 40% except all 220Ω resistors (full opacity). */
  ledButtonFrame11DimWorkspace?: boolean;
  /** Frame 12: workspace at full opacity (re-apply dim restore after Frame 11). */
  ledButtonFrame12Active?: boolean;
  /** Frame 14: dim workspace to 40% except the user’s pushbutton (full opacity). */
  ledButtonFrame14DimWorkspace?: boolean;
  /** Instance id of the real user pushbutton; required when dimming. */
  ledButtonFrame14PushbuttonInstanceId?: string | null;
  /** Frame 12: DOM overlay of push-button hint SVG on breadboard F17–F19 (E/F strip). */
  ledButtonFrame12PushHintOverlay?: boolean;
  /** Frame 15: restore workspace opacity (undo Frame 8 / 11 / 14 tutorial dim). */
  ledButtonFrame15RestoreWorkspaceOpacity?: boolean;
  /**
   * Frame 15: yellow highlight on breadboard bottom (+) rail + solid red vertical guide D19 ↔ PWR_BOTTOM_PLUS_19.
   */
  ledButtonFrame15BottomRailHighlight?: boolean;
  /**
...
export function CircuitCanvas({
  onComponentDrop,
  onComponentSelect,
  ledButtonFrame3PowerDemo = false,
  ledButtonFrame8DimWorkspace = false,
  ledButtonFrame8TargetInstanceId = null,
  ledButtonFrame11DimWorkspace = false,
  ledButtonFrame12Active = false,
  ledButtonFrame14DimWorkspace = false,
  ledButtonFrame14PushbuttonInstanceId = null,
  ledButtonFrame12PushHintOverlay = false,
  ...
  const [viewportRevision, setViewportVersion] = useState(0); // Triggers re-render on viewport change
  /** DOM overlay for LED K.svg — must be state so we re-run after async Fabric image load. */
  const [ledButtonFrame8OverlayStyle, setLedButtonFrame8OverlayStyle] = useState<CSSProperties | null>(null);
  const [frame12PushHintOverlayStyle, setFrame12PushHintOverlayStyle] = useState<CSSProperties | null>(null);
  /** Fabric `div[data-fabric="wrapper"]` — portal mount for LED K so it stacks above the upper canvas. */
  const [fabricWrapperEl, setFabricWrapperEl] = useState<HTMLDivElement | null>(null);

  const ledKSvgUrl = useMemo(() => {
    const base = import.meta.env.BASE_URL ?? '/';
    const normalized = base.endsWith('/') ? base : `${base}/`;
    return `${normalized}projects/led-k.svg`;
  }, []);

  const frame12PushHintSvgUrl = useMemo(() => {
    const base = import.meta.env.BASE_URL ?? '/';
    const normalized = base.endsWith('/') ? base : `${base}/`;
    return `${normalized}projects/led-button-frame12-push-hint.svg`;
  }, []);
  ...
  /** Original Fabric opacities before Frame 8 workspace dim (restored when dim ends). */
  const frame8OpacityBackupRef = useRef<Map<fabric.FabricObject, number>>(new Map());
  /** Original Fabric opacities before Frame 11 workspace dim (restored when dim ends). */
  const frame11OpacityBackupRef = useRef<Map<fabric.FabricObject, number>>(new Map());
  /** Original Fabric opacities before Frame 14 workspace dim (restored when dim ends). */
  const frame14OpacityBackupRef = useRef<Map<fabric.FabricObject, number>>(new Map());
  /** Dedupes LED K overlay state updates from high-frequency `after:render`. */
  const frame8OverlayLayoutKeyRef = useRef<string | null>(null);
  const frame12PushHintLayoutKeyRef = useRef<string | null>(null);
  ...
  /** While dragging breadboard or Arduino Uno — suppress pin hover / accidental wire UI. */
  const draggingUnoOrBreadboardRef = useRef(false);
  /**
   * Breadboard/Uno pins: defer wire start to mouseup so a drag-move does not start a wire.
   * Cleared if pointer moves farther than BASE_PIN_WIRE_DRAG_THRESHOLD_PX before mouseup.
   */
  const pendingWireFromBasePinRef = useRef<{
    componentId: string;
    pinId: string;
    canvasX: number;
    canvasY: number;
  ...
  /** Frame 3 instructional: auto-added power jumpers (removed when demo prop is false) */
  const ledDemoWireIdsRef = useRef<{ fiveV?: string; gnd?: string }>({});
  const ledFrame3RailFabricRef = useRef<fabric.Object[]>([]);
  /** Frame 15: bottom (+) rail fill + D19 vertical guide only */
  const ledFrame15RailFabricRef = useRef<fabric.Object[]>([]);
  /** Frame 17: bottom (−) rail fill + H12 vertical guide only */
  const ledFrame17RailFabricRef = useRef<fabric.Object[]>([]);
  ...
  useEffect(() => {
    ...
    setFabricWrapperEl(fabricCanvas.lowerCanvasEl.parentElement as HTMLDivElement);
    ...
    // Re-render so Frame 8 portal target exists and overlays can measure Fabric objects
  ...
  useEffect(() => {
    ...
    objects.forEach(obj => {
      ...
      if (data?.type === 'component') {
        const instId = data.instanceId as string | undefined;
        const placed = instId
          ? placedComponents.find((c) => c.instanceId === instId)
          : undefined;
        const decorative = !!placed?.decorativeOnly;
        if (wireDrawing.isDrawing) {
  ...
  const findNearestPin = useCallback((
    ...
    for (const component of placedComponents) {
      if (component.decorativeOnly) continue;
  ...
  const handleMouseMoveEvent = useCallback((
    ...
    // Deferred wire start from breadboard/Uno pin: cancel if user is dragging the pointer
    const pending = pendingWireFromBasePinRef.current;
    if (pending && !wireDrawing.isDrawing) {
      const dx = mouseEvent.clientX - pending.startClientX;
      const dy = mouseEvent.clientY - pending.startClientY;
      if (dx * dx + dy * dy > BASE_PIN_WIRE_DRAG_THRESHOLD_PX * BASE_PIN_WIRE_DRAG_THRESHOLD_PX) {
    ...
    // While dragging breadboard/Uno, don't highlight pins (avoids wire affordance during move)
    if (draggingUnoOrBreadboardRef.current) {
      setHoveredPin(null);
      if (wireDrawing.isDrawing) {
        const lastPoint =
          wireDrawing.bendPoints.length > 0
    ...
    for (const wireObj of wireObjects) {
      const wMeta = (wireObj as ComponentFabricObject).data as {
        points?: { x: number; y: number }[];
        overlayAboveComponents?: boolean;
      };
      if (wMeta?.overlayAboveComponents) continue;
      const wireData = wMeta;
    ...
    for (let i = objects.length - 1; i >= 0; i--) {
      ...
      const placedForHover = placedComponents.find(
        (c) => c.instanceId === target.data!.instanceId
      );
      if (placedForHover?.decorativeOnly) continue;
  ...
  }, [placedComponents, getComponentDefinition, setHoveredPin, wireDrawing.isDrawing, wireDrawing.bendPoints, wireDrawing.startX, wireDrawing.startY, updateWireDrawing, getPinCanvasPosition]);
  ...
  const handleMouseDownEvent = useCallback((canvas: fabric.Canvas, mouseEvent: MouseEvent, scenePoint: fabric.Point) => {
    ...
    if (hoveredPin && !selectedWireId && !isSimulating) {
      if (!wireDrawing.isDrawing) {
        const startDef = getComponentDefinition(hoveredPin.componentId);
        const deferWireStartForBase =
          startDef?.id === 'breadboard' || startDef?.id === 'arduino-uno';
        if (deferWireStartForBase) {
          pendingWireFromBasePinRef.current = {
            componentId: hoveredPin.componentId,
            pinId: hoveredPin.pin.id,
            canvasX: hoveredPin.canvasX,
            canvasY: hoveredPin.canvasY,
            startClientX: mouseEvent.clientX,
            startClientY: mouseEvent.clientY,
          };
          // Let Fabric receive the gesture so the board/Uno can be dragged; wire starts on mouseup if it was a click
          return;
        }
        startWireDrawing(
    ...
    // Handle wire click for selection (paths are evented:false; hit-test here)
    const wireObjects = canvas.getObjects().filter(obj => {
      const data = (obj as ComponentFabricObject).data;
      return data?.type === 'wire';
    });

    const deferWireToPart = shouldDeferWireSelectionToComponent(
      canvas,
      mouseEvent,
      getComponentDefinition
    );
    ...
    for (let i = wireObjects.length - 1; i >= 0; i--) {
      ...
      if (!target.data?.wireId) continue;
      if ((target.data as { overlayAboveComponents?: boolean }).overlayAboveComponents) continue;
      if (deferWireToPart) continue;
  ...
  const handleObjectModified = useCallback((obj: ComponentFabricObject) => {
    if (obj?.data?.type === 'component' && obj?.data?.instanceId) {
      const modDef = getComponentDefinition(obj.data.instanceId);
      if (modDef?.id === 'breadboard' || modDef?.id === 'arduino-uno') {
        draggingUnoOrBreadboardRef.current = false;
      }
    }
    ...
    if (obj?.data?.instanceId && obj?.data?.type === 'component') {
      ...
      if (component?.decorativeOnly) {
        const sx = component.x;
        const sy = component.y;
  ...
  const handleObjectMoving = useCallback((obj: ComponentFabricObject) => {
    ...
    const pathString = wirePathStringFromPoints(wire, points);
    const orthoDashBends =
      points.length > 2 && !!(wire.strokeDashArray && wire.strokeDashArray.length > 0);
    ...
    if (wirePath) {
      ...
      const instructionalOverlay = !!wire.overlayAboveComponents;
      const newPath = new fabric.Path(pathString, {
        stroke: wire.color,
        strokeWidth: 6,
        fill: 'transparent',
        strokeLineCap: 'round',
        strokeLineJoin: orthoDashBends ? 'miter' : 'round',
        strokeMiterLimit: orthoDashBends ? 4 : 10,
        strokeDashArray: wire.strokeDashArray?.length ? wire.strokeDashArray : undefined,
        ...
        data: {
          ...
          wireId: wire.id,
          points,
          overlayAboveComponents: instructionalOverlay,
        },
      });
      ...
      if (selectedWireId === wireId) {
        ...
        moveWirePairAboveComponents(canvas, outlinePath, newPath);
      } else {
        moveWirePairAboveComponents(canvas, null, newPath);
      }
    }
  ...
  /** Complete deferred wire start from breadboard/Uno pin (mouseup may occur outside canvas). */
  useEffect(() => {
    const onWindowMouseUp = (e: MouseEvent) => {
      ...
      pendingWireFromBasePinRef.current = null;
      if (useCircuitStore.getState().isSimulating) return;
      const th2 = BASE_PIN_WIRE_DRAG_THRESHOLD_PX * BASE_PIN_WIRE_DRAG_THRESHOLD_PX;
      if (dx * dx + dy * dy > th2) return;
      useCircuitStore.getState().startWireDrawing(
        pending.componentId,
        pending.pinId,
        pending.canvasX,
        pending.canvasY
    ...
    placedComponents.forEach(component => {
      if (!canvasInstanceIds.has(component.instanceId)) {
        ...
        img.onload = () => {
          ...
          const isDecorative = !!component.decorativeOnly;
          const fabricImg = new fabric.FabricImage(img, {
            ...
            lockScalingX: true,
            lockScalingY: true,
            hasBorders: !isDecorative,
            angle: component.rotation,
            flipX: component.flipX || false,
            flipY: component.flipY || false,
            opacity: imageOpacity,
            selectable: !isDecorative,
            evented: !isDecorative,
            lockMovementX: isDecorative,
            lockMovementY: isDecorative,
            hoverCursor: isDecorative ? 'default' : 'move',
          });
          ...
          if (isBaseComponent) {
            ...
            // Wires stack above normal adds; keep ghost / instructional parts visible.
            canvas.bringObjectToFront(fabricImg);
          }

          instanceToFabricMap.current.set(component.instanceId, fabricImg);
          canvas.renderAll();
          // Recompute Frame 8 LED K overlay once Fabric object exists (async image load).
          setViewportVersion((v) => v + 1);
        };
        ...
        if (!isDraggingRef.current) {
          ...
          if (fabricObj) {
            const po = component.properties?.opacity;
            const imageOpacity = typeof po === 'number' ? po : 1;
            fabricObj.set({
              left: component.x,
              top: component.y,
              angle: component.rotation,
  ...
  useEffect(() => {
    ...
    wires.forEach(wire => {
      ...
      const pathString = wirePathStringFromPoints(wire, points);
      /** Dashed multi-segment: orthogonal “half rectangle” — use miter joins at 90° corners. */
      const orthoDashBends =
        points.length > 2 && !!(wire.strokeDashArray && wire.strokeDashArray.length > 0);
      ...
      const wireOpacity = hasError ? 1 : (wire.opacity ?? 1);

      let outlinePath: fabric.Path | null = null;
      if (isSelected) {
        outlinePath = new fabric.Path(pathString, {
      ...
      const dash =
        hasError ? [10, 5] : (wire.strokeDashArray && wire.strokeDashArray.length > 0 ? wire.strokeDashArray : undefined);

      /** Instructional demo wires: skip custom wire selection (see mouse:down). */
      const instructionalOverlay = !!wire.overlayAboveComponents && !hasError;

      // Create the main wire path — always above breadboard/Uno so pins read clearly; evented off
      // so Fabric hits parts below; we select wires manually (unless instructional or small part).
      ...
      canvas.add(path);
      moveWirePairAboveComponents(canvas, outlinePath, path);
      wireToFabricMap.current.set(wire.id, path);
    });

    // Wires are explicitly brought to the front for readability; decorative / instructional
    // parts must sit above those strokes or they disappear.
    for (const comp of placedComponents) {
      if (!comp.decorativeOnly) continue;
      const fab = instanceToFabricMap.current.get(comp.instanceId);
      if (fab) {
        canvas.bringObjectToFront(fab);
      }
    }
  ...
  // ── Button-Powered LED Frame 3: dashed 30% power wires + bottom rail overlays ─
  useEffect(() => {
    const canvas = fabricCanvasRef.current;
    if (!canvas) return;

    const clearRailOverlays = () => {
      ledFrame3RailFabricRef.current.forEach((r) => {
        canvas.remove(r);
      });
      ledFrame3RailFabricRef.current = [];
    };

    if (!ledButtonFrame3PowerDemo) {
      const ids = ledDemoWireIdsRef.current;
      if (ids.fiveV) {
        removeWire(ids.fiveV);
        ids.fiveV = undefined;
      }
      if (ids.gnd) {
        removeWire(ids.gnd);
    ...
    if (ledDemoWireIdsRef.current.fiveV && !wires.some((w) => w.id === ledDemoWireIdsRef.current.fiveV)) {
      ledDemoWireIdsRef.current.fiveV = undefined;
    }
    if (ledDemoWireIdsRef.current.gnd && !wires.some((w) => w.id === ledDemoWireIdsRef.current.gnd)) {
      ledDemoWireIdsRef.current.gnd = undefined;
    }
    ...
    const bbDef = getComponentDefinition(bb.instanceId);
    const bbFabric = instanceToFabricMap.current.get(bb.instanceId);
    if (bbDef && bbFabric) {
      const pathPlus = breadboardRailLocalRectToCanvasPathD(
        bbFabric,
        bbDef,
        BREADBOARD_BOTTOM_RAIL_LOCAL.plus
      );
      const pPlus = new fabric.Path(pathPlus, {
        fill: 'rgba(255, 220, 80, 0.5)',
        strokeWidth: 0,
        selectable: false,
        evented: false,
        perPixelTargetFind: false,
        data: { type: 'led-frame3-rail-highlight' },
      });
      canvas.add(pPlus);
      ledFrame3RailFabricRef.current.push(pPlus);

      const pathMinus = breadboardRailLocalRectToCanvasPathD(
        bbFabric,
        bbDef,
        BREADBOARD_BOTTOM_RAIL_LOCAL.minus
      );
      const pMinus = new fabric.Path(pathMinus, {
      ...
      canvas.add(pMinus);
      ledFrame3RailFabricRef.current.push(pMinus);
    }
    ...
    if (bbFabric) {
      ...
      if (bbIdx >= 0) {
        ledFrame3RailFabricRef.current.forEach((pathObj) => {
          canvas.moveObjectTo(pathObj, bbIdx + 1);
        });
      }
    }

    const pos5vBb = getPinCanvasPosition(bb.instanceId, 'PWR_BOTTOM_PLUS_14');
    const pos5vUno = getPinCanvasPosition(uno.instanceId, '5V');
    const posGndBb = getPinCanvasPosition(bb.instanceId, 'PWR_BOTTOM_MINUS_16');
    const posGndUno = getPinCanvasPosition(uno.instanceId, 'GND1');

    const hasUser5v = wires.some(
      (w) =>
        (w.startComponentId === bb.instanceId &&
          w.startPinId === 'PWR_BOTTOM_PLUS_14' &&
          w.endComponentId === uno.instanceId &&
          w.endPinId === '5V') ||
        (w.endComponentId === bb.instanceId &&
          w.endPinId === 'PWR_BOTTOM_PLUS_14' &&
          w.startComponentId === uno.instanceId &&
          w.startPinId === '5V')
    );
    const hasUserGnd = wires.some(
      (w) =>
        (w.startComponentId === bb.instanceId &&
          w.startPinId === 'PWR_BOTTOM_MINUS_16' &&
          w.endComponentId === uno.instanceId &&
          w.endPinId === 'GND1') ||
        (w.endComponentId === bb.instanceId &&
          w.endPinId === 'PWR_BOTTOM_MINUS_16' &&
          w.startComponentId === uno.instanceId &&
          w.startPinId === 'GND1')
    );

    if (pos5vBb && pos5vUno && !ledDemoWireIdsRef.current.fiveV && !hasUser5v) {
      const id = addWire({
        startComponentId: bb.instanceId,
        startPinId: 'PWR_BOTTOM_PLUS_14',
        endComponentId: uno.instanceId,
        endPinId: '5V',
        bendPoints: buildLedPowerRailJogBends(pos5vBb, pos5vUno, LED_RAIL_DEMO_RUN_OFFSET_5V),
        color: '#dc2626',
        opacity: 0.3,
        strokeDashArray: [8, 6],
        overlayAboveComponents: true,
      });
      ledDemoWireIdsRef.current.fiveV = id;
    }

    if (posGndBb && posGndUno && !ledDemoWireIdsRef.current.gnd && !hasUserGnd) {
      const id = addWire({
        startComponentId: bb.instanceId,
        startPinId: 'PWR_BOTTOM_MINUS_16',
        endComponentId: uno.instanceId,
        endPinId: 'GND1',
        bendPoints: buildLedPowerRailJogBends(posGndBb, posGndUno, LED_RAIL_DEMO_RUN_OFFSET_GND),
        color: '#000000',
        opacity: 0.3,
        strokeDashArray: [8, 6],
        overlayAboveComponents: true,
      });
      ledDemoWireIdsRef.current.gnd = id;
    }

    // Rails are inserted just above the breadboard; re-stack demo wires on top so they stay visible on pins
    for (const wid of [ledDemoWireIdsRef.current.fiveV, ledDemoWireIdsRef.current.gnd] as const) {
      if (!wid) continue;
      const fp = wireToFabricMap.current.get(wid);
      if (fp) canvas.bringObjectToFront(fp);
    }
    canvas.renderAll();
  }, [
    ...
    placedComponents,
    wires,
    getPinCanvasPosition,
    addWire,
    removeWire,
  ]);

  // ── Button-Powered LED Frame 15: bottom (+) rail yellow + D19 ↓ PWR_BOTTOM_PLUS_19 solid red guide ─
  useEffect(() => {
    const canvas = fabricCanvasRef.current;
    if (!canvas) return;

    const clearFrame15Overlays = () => {
      ledFrame15RailFabricRef.current.forEach((r) => {
        canvas.remove(r);
      });
      ledFrame15RailFabricRef.current = [];
    };

    if (!ledButtonFrame15BottomRailHighlight) {
      clearFrame15Overlays();
      canvas.renderAll();
      return;
    }
    ...
    const bbFabric = instanceToFabricMap.current.get(bb.instanceId);
    if (bbDef && bbFabric) {
      const pathBottomPlus = breadboardRailLocalRectToCanvasPathD(
        bbFabric,
        bbDef,
        BREADBOARD_BOTTOM_RAIL_LOCAL.plus
      );
      const pBottomPlus = new fabric.Path(pathBottomPlus, {
        fill: 'rgba(255, 220, 80, 0.55)',
        strokeWidth: 0,
        selectable: false,
        evented: false,
        perPixelTargetFind: false,
        data: { type: 'led-frame15-rail-highlight' },
      });
      canvas.add(pBottomPlus);
      ledFrame15RailFabricRef.current.push(pBottomPlus);

      const posD19 = getPinCanvasPosition(bb.instanceId, 'D19');
      const posBottomPlus19 = getPinCanvasPosition(bb.instanceId, 'PWR_BOTTOM_PLUS_19');
      if (posD19 && posBottomPlus19) {
        const xMid = (posD19.x + posBottomPlus19.x) / 2;
        const yTop = Math.min(posD19.y, posBottomPlus19.y);
        const yBot = Math.max(posD19.y, posBottomPlus19.y);
        const d19Guide = new fabric.Line([xMid, yTop, xMid, yBot], {
          stroke: '#dc2626',
          strokeWidth: 4,
          opacity: 0.5,
          selectable: false,
        ...
        canvas.add(d19Guide);
        ledFrame15RailFabricRef.current.push(d19Guide);
      }
    }
    ...
    if (bbFabric) {
      ...
      const bbIdx = objs.indexOf(bbFabric);
      if (bbIdx >= 0) {
        ledFrame15RailFabricRef.current.forEach((obj) => {
          canvas.moveObjectTo(obj, bbIdx + 1);
        });
      }
    }

    for (const o of ledFrame15RailFabricRef.current) {
    ...
    canvas.renderAll();
  }, [ledButtonFrame15BottomRailHighlight, placedComponents, getPinCanvasPosition]);

  // ── Button-Powered LED Frame 17: bottom (−) rail blue + H12 ↓ PWR_BOTTOM_MINUS_12 black guide ─
  useEffect(() => {
    const canvas = fabricCanvasRef.current;
    if (!canvas) return;

    const clearFrame17Overlays = () => {
      ledFrame17RailFabricRef.current.forEach((r) => {
        canvas.remove(r);
      });
      ledFrame17RailFabricRef.current = [];
    };

    if (!ledButtonFrame17BottomMinusHighlight) {
      clearFrame17Overlays();
      canvas.renderAll();
      return;
    }
    ...
    const bbFabric = instanceToFabricMap.current.get(bb.instanceId);
    if (bbDef && bbFabric) {
      const pathBottomMinus = breadboardRailLocalRectToCanvasPathD(
        bbFabric,
        bbDef,
        BREADBOARD_BOTTOM_RAIL_LOCAL.minus
      );
      const pBottomMinus = new fabric.Path(pathBottomMinus, {
        fill: 'rgba(80, 170, 255, 0.5)',
        strokeWidth: 0,
        selectable: false,
        evented: false,
        perPixelTargetFind: false,
        data: { type: 'led-frame17-rail-highlight' },
      });
      canvas.add(pBottomMinus);
      ledFrame17RailFabricRef.current.push(pBottomMinus);

      const posH12 = getPinCanvasPosition(bb.instanceId, 'H12');
      const posBottomMinus12 = getPinCanvasPosition(bb.instanceId, 'PWR_BOTTOM_MINUS_12');
      if (posH12 && posBottomMinus12) {
        const xMid = (posH12.x + posBottomMinus12.x) / 2;
        const yTop = Math.min(posH12.y, posBottomMinus12.y);
        const yBot = Math.max(posH12.y, posBottomMinus12.y);
        const h12Guide = new fabric.Line([xMid, yTop, xMid, yBot], {
          stroke: '#000000',
          strokeWidth: 4,
          opacity: 0.4,
          selectable: false,
        ...
        canvas.add(h12Guide);
        ledFrame17RailFabricRef.current.push(h12Guide);
      }
    }
    ...
    if (bbFabric) {
      ...
      const bbIdx = objs.indexOf(bbFabric);
      if (bbIdx >= 0) {
        ledFrame17RailFabricRef.current.forEach((obj) => {
          canvas.moveObjectTo(obj, bbIdx + 1);
        });
      }
    }

    for (const o of ledFrame17RailFabricRef.current) {
      if (o instanceof fabric.Line) {
        canvas.bringObjectToFront(o);
      }
    }
    ...
    canvas.renderAll();
  }, [ledButtonFrame17BottomMinusHighlight, placedComponents, getPinCanvasPosition]);

  const applyLedButtonFrame8Dim = useCallback(() => {
    const canvas = fabricCanvasRef.current;
    if (!canvas) return;

    frame8OpacityBackupRef.current.forEach((op, obj) => {
      obj.set('opacity', op);
    });
    frame8OpacityBackupRef.current.clear();

    if (!ledButtonFrame8DimWorkspace || !ledButtonFrame8TargetInstanceId) {
      canvas.renderAll();
      return;
    }
    ...
    canvas.getObjects().forEach((obj) => {
      ...
      const placed = instId ? placedComponents.find((c) => c.instanceId === instId) : undefined;
      const definition = instId ? getComponentDefinition(instId) : undefined;
      const isUserPlacedLed5mm =
        !!placed && !placed.decorativeOnly && definition?.id === 'led-5mm';

      const exempt =
        (data?.type === 'component' && isUserPlacedLed5mm) ||
        (data?.type === 'led-overlay' && isUserPlacedLed5mm) ||
        (data?.type === 'onboarding' && isUserPlacedLed5mm);
      const current = obj.opacity ?? 1;
      frame8OpacityBackupRef.current.set(obj, current);
      if (!exempt) {
        obj.set('opacity', current * DIM);
      }
    });
    canvas.renderAll();
  }, [ledButtonFrame8DimWorkspace, ledButtonFrame8TargetInstanceId, placedComponents, getComponentDefinition]);

  const applyLedButtonFrame11Dim = useCallback(() => {
    const canvas = fabricCanvasRef.current;
    if (!canvas) return;

    frame11OpacityBackupRef.current.forEach((op, obj) => {
      obj.set('opacity', op);
    });
    frame11OpacityBackupRef.current.clear();

    if (!ledButtonFrame11DimWorkspace) {
      canvas.renderAll();
      return;
    }
    ...
    canvas.getObjects().forEach((obj) => {
      ...
      const placed = instId ? placedComponents.find((c) => c.instanceId === instId) : undefined;
      const definition = instId ? getComponentDefinition(instId) : undefined;
      const isResistor = !!placed && definition?.id === 'registor_220ω';

      const exempt =
        (data?.type === 'component' && isResistor) ||
        (data?.type === 'onboarding' && isResistor);
      const current = obj.opacity ?? 1;
      frame11OpacityBackupRef.current.set(obj, current);
      if (!exempt) {
        obj.set('opacity', current * DIM);
      }
    });
    canvas.renderAll();
  }, [ledButtonFrame11DimWorkspace, placedComponents, getComponentDefinition]);

  const applyLedButtonFrame14Dim = useCallback(() => {
    const canvas = fabricCanvasRef.current;
    if (!canvas) return;

    frame14OpacityBackupRef.current.forEach((op, obj) => {
      obj.set('opacity', op);
    });
    frame14OpacityBackupRef.current.clear();

    if (!ledButtonFrame14DimWorkspace || !ledButtonFrame14PushbuttonInstanceId) {
      canvas.renderAll();
      return;
    }

    const DIM = 0.4;
    const targetId = ledButtonFrame14PushbuttonInstanceId;
    ...
    canvas.getObjects().forEach((obj) => {
      ...
      const placed = instId ? placedComponents.find((c) => c.instanceId === instId) : undefined;
      const definition = instId ? getComponentDefinition(instId) : undefined;
      const isUserTargetPushbutton =
        instId === targetId &&
        !!placed &&
        !placed.decorativeOnly &&
        definition?.id === 'pushbutton';

      const exempt =
        (data?.type === 'component' && isUserTargetPushbutton) ||
        (data?.type === 'onboarding' && isUserTargetPushbutton);
      const current = obj.opacity ?? 1;
      frame14OpacityBackupRef.current.set(obj, current);
      if (!exempt) {
        obj.set('opacity', current * DIM);
      }
    });
    ...
    ledButtonFrame14PushbuttonInstanceId,
    placedComponents,
    getComponentDefinition,
  ]);

  const updateLedButtonFrame8Overlay = useCallback(() => {
    if (!ledButtonFrame8DimWorkspace || !ledButtonFrame8TargetInstanceId) {
      frame8OverlayLayoutKeyRef.current = null;
      setLedButtonFrame8OverlayStyle(null);
      return;
    }
    const canvas = fabricCanvasRef.current;
    const obj = instanceToFabricMap.current.get(ledButtonFrame8TargetInstanceId);
    if (!canvas || !obj) {
      frame8OverlayLayoutKeyRef.current = null;
      setLedButtonFrame8OverlayStyle(null);
      return;
    ...
    const corners = obj.getCoords().map((p) => fabric.util.transformPoint(p, vpt));
    let minX = Infinity;
    let minY = Infinity;
    let maxX = -Infinity;
    let maxY = -Infinity;
    ...
    const layoutKey = `${minX.toFixed(2)},${minY.toFixed(2)},${w.toFixed(2)},${h.toFixed(2)}`;
    if (frame8OverlayLayoutKeyRef.current === layoutKey) return;
    frame8OverlayLayoutKeyRef.current = layoutKey;
    setLedButtonFrame8OverlayStyle({
      position: 'absolute',
      left: minX,
      top: minY,
      width: w,
  ...
  }, [ledButtonFrame8DimWorkspace, ledButtonFrame8TargetInstanceId]);

  const updateLedButtonFrame12PushHintOverlay = useCallback(() => {
    if (!ledButtonFrame12PushHintOverlay) {
      frame12PushHintLayoutKeyRef.current = null;
      setFrame12PushHintOverlayStyle(null);
      return;
    }
    const canvas = fabricCanvasRef.current;
    const bb = placedComponents.find((c) => c.definitionId === 'breadboard');
    ...
    const pinIds = ['F17', 'F19', 'E17', 'E19'] as const;
    const scenePts: { x: number; y: number }[] = [];
    for (const pinId of pinIds) {
      const p = getPinCanvasPosition(bb.instanceId, pinId);
      if (!p) {
        frame12PushHintLayoutKeyRef.current = null;
        setFrame12PushHintOverlayStyle(null);
        return;
    ...
    const vpt = canvas.viewportTransform ?? fabric.iMatrix;
    const screenPts = scenePts.map((pt) =>
      fabric.util.transformPoint(new fabric.Point(pt.x, pt.y), vpt)
    );
    const pad = 6;
    let minX = Infinity;
    let minY = Infinity;
    ...
    const h = Math.max(1, maxY - minY);
    const layoutKey = `${minX.toFixed(2)},${minY.toFixed(2)},${w.toFixed(2)},${h.toFixed(2)}`;
    if (frame12PushHintLayoutKeyRef.current === layoutKey) return;
    frame12PushHintLayoutKeyRef.current = layoutKey;
    setFrame12PushHintOverlayStyle({
      position: 'absolute',
      left: minX,
      top: minY,
      width: w,
  ...
  }, [ledButtonFrame12PushHintOverlay, placedComponents, getPinCanvasPosition]);

  useEffect(() => {
    updateLedButtonFrame8Overlay();
  }, [
    updateLedButtonFrame8Overlay,
    viewportRevision,
    zoom,
  ...
  useEffect(() => {
    updateLedButtonFrame12PushHintOverlay();
  }, [
    updateLedButtonFrame12PushHintOverlay,
    viewportRevision,
    zoom,
  ...
  }, [updateLedButtonFrame8Overlay, updateLedButtonFrame12PushHintOverlay]);

  useEffect(() => {
    applyLedButtonFrame8Dim();

    const canvas = fabricCanvasRef.current;
    if (!canvas || !ledButtonFrame8DimWorkspace || !ledButtonFrame8TargetInstanceId) {
      return () => {
    ...
    canvas.on('object:added', onStructureChange);
    canvas.on('object:removed', onStructureChange);

    return () => {
      canvas.off('object:added', onStructureChange);
      canvas.off('object:removed', onStructureChange);
      const c = fabricCanvasRef.current;
      if (c) {
        frame8OpacityBackupRef.current.forEach((op, obj) => {
          obj.set('opacity', op);
  ...
  useEffect(() => {
    applyLedButtonFrame11Dim();

    const canvas = fabricCanvasRef.current;
    if (!canvas || !ledButtonFrame11DimWorkspace) {
      return () => {
        const c = fabricCanvasRef.current;
        if (c) {
          frame11OpacityBackupRef.current.forEach((op, obj) => {
  ...
  useEffect(() => {
    if (!ledButtonFrame12Active) return;
    applyLedButtonFrame11Dim();
  }, [ledButtonFrame12Active, applyLedButtonFrame11Dim]);

  useEffect(() => {
    if (!ledButtonFrame15RestoreWorkspaceOpacity) return;
    applyLedButtonFrame8Dim();
    applyLedButtonFrame11Dim();
    applyLedButtonFrame14Dim();
    fabricCanvasRef.current?.renderAll();
  }, [
  ...
  useEffect(() => {
    ...
    const canvas = fabricCanvasRef.current;
    if (!canvas || !ledButtonFrame14DimWorkspace || !ledButtonFrame14PushbuttonInstanceId) {
      return () => {
        const c = fabricCanvasRef.current;
        if (c) {
          frame14OpacityBackupRef.current.forEach((op, obj) => {
  ...
  useEffect(() => {
    const canvas = fabricCanvasRef.current;
    if (!canvas || !ledButtonFrame8DimWorkspace || !ledButtonFrame8TargetInstanceId) return;
  ...
  // ── Design-mode: wire-path SVG highlights only (no ball) ─────────────────
  // Do not add gold Fabric rects on breadboard rows here — traceAllConnected
  // places many net highlights that look like stray yellow bars and stack over
  // the workspace. Instructional rail overlays use led-frame3 paths only.
  ...
  useEffect(() => {
    ...
    // Clear any leftover Fabric breadboard gold rects from sim or older builds
    // (design mode never uses bbHighlightObjectsRef for tracing).
    let fabricChanged = false;
    if (canvas && bbHighlightObjectsRef.current.size > 0) {
      bbHighlightObjectsRef.current.forEach((obj) => canvas.remove(obj));
      bbHighlightObjectsRef.current.clear();
      fabricChanged = true;
      canvas.renderAll();
  ...
  useEffect(() => {
    ...
    if (wireDrawing.isDrawing) {
      ...
      routeRafRef.current = requestAnimationFrame(() => {
        ...
        const end: RouterPoint = {
          x: Math.round(wireDrawing.currentX / WIRE_PREVIEW_END_SNAP_PX) * WIRE_PREVIEW_END_SNAP_PX,
          y: Math.round(wireDrawing.currentY / WIRE_PREVIEW_END_SNAP_PX) * WIRE_PREVIEW_END_SNAP_PX,
        };
      ...
      if (item.type === 'component') {
        ...
        if (wirePath) {
          ...
          if (wireData?.points) {
            ...
            const storeWire = useCircuitStore.getState().wires.find(w => w.id === item.id);
            const pathString = wirePathStringFromPoints(storeWire ?? {}, points);
            const orthoDashBends =
              points.length > 2 &&
              !!(storeWire?.strokeDashArray && storeWire.strokeDashArray.length > 0);
  ...
  useEffect(() => {
    ...
    // Manual onboarding (e.g. Frame 8 reopen): cancel pending auto-fade and keep guide at full opacity
    for (const [instanceId, entry] of activeOnboardings) {
      if (!entry.manual) continue;
      const timer = onboardingFadeTimersRef.current.get(instanceId);
      if (timer) {
        clearTimeout(timer);
        onboardingFadeTimersRef.current.delete(instanceId);
      }
  ...
  const createComponentWithImage = useCallback(async (
    ...
    img.onload = () => {
      ...
      const placed = useCircuitStore.getState().placedComponents.find((c) => c.instanceId === instanceId);
      const po = placed?.properties?.opacity;
      const imageOpacity = typeof po === 'number' ? po : 1;
  ...
  useEffect(() => {
    placedComponents.forEach(component => {
      ...
      if (isLEDComponent && !component.decorativeOnly) {
  ...
  return (
    <div className="circuit-canvas-container">
      ...
      >
        ...
        {fabricWrapperEl && ledButtonFrame8OverlayStyle
          ? createPortal(
              <img alt="" src={ledKSvgUrl} draggable={false} style={ledButtonFrame8OverlayStyle} />,
              fabricWrapperEl
            )
          : null}

        {fabricWrapperEl && frame12PushHintOverlayStyle
          ? createPortal(
              <img
                alt=""
                src={frame12PushHintSvgUrl}
                draggable={false}
                style={frame12PushHintOverlayStyle}
              />,
              fabricWrapperEl
            )
          : null}
        ...
        {hoveredPin && !selectedWireId && (() => {
          ...
          // Build label with optional polarity indicator (prefer definition pin labels, e.g. "Anode (+)")
          const pinFromDef = definition?.pins?.find((p) => p.id === hoveredPin.pin.id);
          let pinLabel = pinFromDef?.label ?? hoveredPin.pin.id;
          if (isLED) {
            if (hoveredPin.pin.id.toUpperCase() === 'ANODE' || hoveredPin.pin.type === 'power') {
              pinLabel = pinFromDef?.label ?? `${hoveredPin.pin.id} (+)`;
            } else if (hoveredPin.pin.id.toUpperCase() === 'CATHODE' || hoveredPin.pin.type === 'ground') {
              pinLabel = pinFromDef?.label ?? `${hoveredPin.pin.id} (–)`;
            }
          }
          const isLedAnodeTooltip =
            isLED && hoveredPin.pin.id.toUpperCase() === 'ANODE';
          ...
          return (
            <>
              ...
              {!isBreadboard && (
                ...
                >
                  {isLedAnodeTooltip ? (
                    <span className="pin-tooltip-led-anode">{pinLabel}</span>
                  ) : (
                    pinLabel
                  )}
                </div>
