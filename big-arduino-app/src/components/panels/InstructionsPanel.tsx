...
/** When all five starter parts are on the canvas (LED Button project step 1). Exported for layout automation. */
export function isLedButtonWorkspaceStarterComplete(
  state: Pick<StepCheckState, 'placedComponents'>
): boolean {
  const required = ['arduino-uno', 'breadboard', 'led-5mm', 'pushbutton', 'registor_220\u03C9'];
  const real = state.placedComponents.filter((c) => !c.decorativeOnly);
  const placedLower = real.map((c) => c.definitionId.toLowerCase());
...
export const LIGHT_UP_LED_STEPS: InstructionStep[] = [
  {
    ...
    checkCompletion: (state) => isLedButtonWorkspaceStarterComplete(state),
  },
  {
    id: 'step-2',
    ...
    checkCompletion: (state) => isPowerRailActive(state) && isGNDRailActive(state),
  },
  {
    id: 'step-3',
    title: 'Place the LED',
  ...
  },
  {
    ...
    description:
      'Position the push button on the breadboard.\n\n\u2022 Place the button onto the breadboard. Insert one leg into the same row as the other leg of the resistor.\n\nTip: It\'s best to place the button across the center gap of the breadboard so the two sides are separated correctly.',
    encouragingMessage: 'Nice! The button is in place.',
    checkCompletion: (state) => {
      return state.placedComponents.some(
        (c) =>
          c.definitionId === 'pushbutton' &&
          !c.decorativeOnly &&
          c.parentBreadboardId
      );
    },
  },
  {
    ...
    checkCompletion: (state) => {
      const button = state.placedComponents.find(
        (c) => c.definitionId === 'pushbutton' && !c.decorativeOnly
      );
  ...
  },
  {
    ...
    checkCompletion: (state) => {
      const led = state.placedComponents.find(
        (c) => c.definitionId === 'led-5mm' && !c.decorativeOnly
      );
...
interface InstructionsPanelProps {
  ...
  /** When true, no visible UI (used when Instructions is omitted from the dock but toolbar step highlights should still run). */
  headless?: boolean;
}
...
export function InstructionsPanel({
  ...
  // Headless: no Next button — advance when the current step is satisfied so highlights match progress
  useEffect(() => {
    if (!headless) return;
    if (!isCurrentStepComplete) return;
    if (currentStepIndex >= steps.length - 1) return;
    setCurrentStepIndex((i) => i + 1);
  }, [headless, isCurrentStepComplete, currentStepIndex, steps.length]);
  ...
  if (headless) {
    return (
      <div
        ref={rootRef}
        className="instructions-panel instructions-panel--headless"
        aria-hidden="true"
      />
    );
  }
