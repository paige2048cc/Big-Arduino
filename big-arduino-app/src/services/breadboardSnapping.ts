...
/**
 * Placement for a two-pin component when breadboard holes are fixed (instructional / scripted).
 * Keys in `insertedPins` must be component pin ids (e.g. ANODE / CATHODE for LEDs).
 */
export function computeSnapPositionForBreadboardPins(
  componentDef: ComponentDefinition,
  breadboard: PlacedComponent,
  breadboardDef: ComponentDefinition,
  ...
  flipX = false,
  flipY = false,
  /** Allow scripted placement when hole spacing differs from component pin pitch (default 10). */
  maxSeparationErrorPx = 10
): SnapResult | null {
  const pins = componentDef.pins;
  if (pins.length !== 2) return null;
  const [pin1, pin2] = pins;
  const bbId1 = insertedPins[pin1.id];
  const bbId2 = insertedPins[pin2.id];
  if (!bbId1 || !bbId2) return null;
  const bb1 = breadboardDef.pins.find((p) => p.id === bbId1);
  const bb2 = breadboardDef.pins.find((p) => p.id === bbId2);
  if (!bb1?.net || !bb2?.net) return null;
  if (!areSameRowSection(bb1.net, bb2.net)) return null;

  const target1 = transformToCanvas(
    bb1.x,
    bb1.y,
    breadboard.x,
    breadboard.y,
  ...
  const target2 = transformToCanvas(
    bb2.x,
    bb2.y,
    breadboard.x,
    breadboard.y,
  ...
  const expectedSep = distance({ x: pin1.x, y: pin1.y }, { x: pin2.x, y: pin2.y });
  const actualSep = distance(target1, target2);
  if (Math.abs(actualSep - expectedSep) > maxSeparationErrorPx) return null;
  ...
  return {
    success: true,
    snappedPosition,
    insertedPins: { [pin1.id]: bbId1, [pin2.id]: bbId2 },
    breadboardInstanceId: breadboard.instanceId,
  };
}

/**
 * Scripted placement for a 4-pin component (e.g. pushbutton) when all breadboard holes are fixed.
 * Verifies all four pins align after solving position from two anchor pins (PIN1B / PIN2B).
 */
export function computeFourPinSnapPositionForBreadboardPins(
  componentDef: ComponentDefinition,
  breadboard: PlacedComponent,
  breadboardDef: ComponentDefinition,
  insertedPins: Record<string, string>,
  ...
  for (const pin of pins) {
    const bbId = insertedPins[pin.id];
    if (!bbId) return null;
    const bb = breadboardDef.pins.find((p) => p.id === bbId);
    if (!bb) return null;
    targets[pin.id] = transformToCanvas(
      bb.x,
      bb.y,
      breadboard.x,
      breadboard.y,
  ...
  const pin1b = pins.find((p) => p.id === 'PIN1B');
  const pin2b = pins.find((p) => p.id === 'PIN2B');
  if (!pin1b || !pin2b) return null;

  const expectedSep = distance({ x: pin1b.x, y: pin1b.y }, { x: pin2b.x, y: pin2b.y });
  const actualSep = distance(targets['PIN1B'], targets['PIN2B']);
  // Breadboard hole spacing can differ slightly from component pin pitch (e.g. F17–E17 vs PIN1B–PIN2B).
  if (Math.abs(actualSep - expectedSep) > 18) return null;
  ...
  for (const pin of pins) {
    ...
    if (distance(pos, targets[pin.id]) > maxPinErrorPx) {
      return null;
    }
  }
