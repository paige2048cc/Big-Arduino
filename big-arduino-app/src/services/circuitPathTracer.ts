...
function traceFrom(
  ...
  if (comp.decorativeOnly) return EMPTY;
  ...
  if (pin?.net) {
    ...
    for (const netPin of netPins) {
      ...
      for (const insertedComp of components) {
        ...
        if (insertedComp.decorativeOnly) continue;
  ...
  if (comp.parentBreadboardId && comp.insertedPins && !comp.decorativeOnly) {
