...
function tracePath(
  ...
  const placedStart = components.find((c) => c.instanceId === startComponentId);
  if (placedStart?.decorativeOnly) {
    return result;
  }
    ...
    if (currentBBPin?.net) {
      ...
      for (const sibling of netSiblings) {
        ...
        for (const insertedComponent of components) {
          ...
          if (insertedComponent.decorativeOnly) continue;
  ...
  if (
    ...
    currentComponent.insertedPins &&
    !currentComponent.decorativeOnly
  ) {
...
export function analyzeCircuit(
  ...
  for (const component of components) {
    ...
    // Validate LEDs (skip decorative ghost overlays)
    if (isLED(componentType) && !component.decorativeOnly) {
