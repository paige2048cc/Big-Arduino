...
interface CircuitActions {
  ...
  removeWire: (wireId: string) => void;
    updateWire: (wireId: string, updates: Partial<Pick<Wire, 'bendPoints' | 'color' | 'opacity' | 'strokeDashArray' | 'overlayAboveComponents' | 'startComponentId' | 'startPinId' | 'endComponentId' | 'endPinId'>>) => void;
...
export const useCircuitStore = create<CircuitState & CircuitActions>()(
  immer((set, get) => ({
    ...
    // Component actions
    addComponent: (definition, x, y, properties = {}, decorativeOnly = false) => {
      get().pushToHistory();
      ...
      set((state) => {
        state.placedComponents.push({
          ...
          ...(decorativeOnly ? { decorativeOnly: true } : {}),
        });
    ...
    completeWireDrawing: (endComponentId, endPinId, bendPointsOverride) => {
      ...
      // Check if a *user* wire already exists (instructional dashed / overlay wires do not block duplicates)
      const { wires } = get();
      const exists = wires.some((w) => {
        if (w.overlayAboveComponents) return false;
        return (
    ...
    addWire: (wire) => {
      ...
      set((state) => {
        state.wires.push({
          ...
          overlayAboveComponents: wire.overlayAboveComponents,
        });
      });
      ...
      set((state) => {
        ...
        if (wire) {
          ...
          if (updates.opacity !== undefined) {
            wire.opacity = updates.opacity;
          }
          if (updates.strokeDashArray !== undefined) {
            wire.strokeDashArray = updates.strokeDashArray;
          }
          if (updates.overlayAboveComponents !== undefined) {
            wire.overlayAboveComponents = updates.overlayAboveComponents;
          }
        }
      });
