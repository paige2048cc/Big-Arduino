...
export interface PlacedComponent {
  ...
  /** Visual-only overlay: no drag/select, no simulation, not counted as a real part */
...
export interface Wire {
  ...
  // Wire color
  color: string;
  /** 0–1; default 1. Used for instructional / dashed power jumpers */
  opacity?: number;
  /** e.g. [8, 6] for dashed; undefined = solid (unless simulation error styling applies) */
  strokeDashArray?: number[];
  /**
   * Draw above components so endpoints read clearly on pins; path uses evented:false
   * so clicks pass through to breadboard/components (e.g. pushbutton).
   */
